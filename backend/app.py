...
# ── PAGE CONFIG ──────────────────────────────
st.set_page_config(
    page_title="TalentIQ — AI Resume Evaluator",
    page_icon="⭐",
    layout="wide"
)
...
.question-card { background: #f8f9fa; border-radius: 10px; padding: 16px; margin: 8px 0; border: 1px solid #e0e0e0; }
...
if st.button("🚀 Evaluate Candidate", type="primary", use_container_width=True):
    ...
    if "YOUR_GEMINI" in GEMINI_API_KEY:
        st.error("❌ Please paste your Gemini API key on line 9 of app.py")
        st.stop()
    ...
    try:
        ...
        from engine_gemini import parse_resume
        ...
        from engine_gemini import score_candidate
        ...
        from engine_gemini import verify_claims
        ...
        from engine_gemini import classify_tier
        ...
        from engine_gemini import generate_interview_questions
        ...
        with s1:
            st.markdown(f"""<div class="score-box">
                <div class="score-num" style="color:#1D9E75">{scores.exact_match:.0f}</div>
                <div class="score-label">Exact Match</div>
            </div>""", unsafe_allow_html=True)
        with s2:
            st.markdown(f"""<div class="score-box">
                <div class="score-num" style="color:#378ADD">{scores.semantic_similarity:.0f}</div>
                <div class="score-label">Semantic Similarity</div>
            </div>""", unsafe_allow_html=True)
        with s3:
            st.markdown(f"""<div class="score-box">
                <div class="score-num" style="color:#BA7517">{scores.achievement_impact:.0f}</div>
                <div class="score-label">Achievements</div>
            </div>""", unsafe_allow_html=True)
        with s4:
            st.markdown(f"""<div class="score-box">
                <div class="score-num" style="color:#533AB7">{scores.ownership_leadership:.0f}</div>
                <div class="score-label">Ownership</div>
            </div>""", unsafe_allow_html=True)
        with s5:
            st.markdown(f"""<div class="score-box">
                <div class="score-num" style="color:#333">{scores.composite:.1f}</div>
        ...
        with tab1:
            for dim, expl, color in [
                ("Exact Match", explanation.exact_match, "#1D9E75"),
                ("Semantic Similarity", explanation.semantic_similarity, "#378ADD"),
                ("Achievement Impact", explanation.achievement_impact, "#BA7517"),
                ("Ownership & Leadership", explanation.ownership_leadership, "#533AB7"),
            ]:
        ...
        # ── DOWNLOAD JSON ─────────────────────────
        st.divider()
        from engine_gemini import FullEvaluation
        ...
        status.empty()
        if "429" in str(e):
            st.error("⏳ Rate limit hit — wait 60 seconds and try again (Gemini free tier limit)")
        else:
