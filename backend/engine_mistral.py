"""
TalentIQ Core Engine (Mistral Free Tier Version)
-------------------------------------------------
Uses Mistral AI free API.
...
def call_mistral(prompt: str) -> dict:
    payload = json.dumps({
        ...
        "messages": [
            {
                "role": "system",
                "content": "You are a helpful assistant that always responds with valid JSON only. Never include markdown, code fences, or any text outside the JSON."
            },
            {
                "role": "user",
                "content": prompt
...
def call_mistral_list(prompt: str) -> list:
    """Special call for endpoints that return a JSON array."""
    payload = json.dumps({
        "model": "mistral-small-latest",
        "messages": [
            {
                "role": "system",
                "content": "You are a helpful assistant. Return ONLY a valid JSON array. No markdown, no code fences, no text outside the array."
            },
            {
                "role": "user",
                "content": prompt
...
- experience_years: calculate from dates
...
{resume_text}"""
    ...
    data = call_mistral(prompt)

    def ensure_str_list(val):
        if not isinstance(val, list):
            return []
        return [str(x) if not isinstance(x, str) else x for x in val]

    return CandidateProfile(
        name=str(data.get("name", "Unknown")),
        email=str(data.get("email", "")),
        skills=ensure_str_list(data.get("skills", [])),
        experience_years=float(data.get("experience_years", 0)),
        roles=ensure_str_list(data.get("roles", [])),
        education=str(data.get("education", "")),
        github_url=data.get("github_url"),
        linkedin_url=data.get("linkedin_url"),
        achievements=ensure_str_list(data.get("achievements", [])),
...
{{"consistency_score": 78, "consistency_notes": ["note1", "note2"]}}"""
    ...
    try:
        cdata = call_mistral(consistency_prompt)
...
JD: {jd_text[:600]}"""
    ...
    items = call_mistral_list(prompt)
